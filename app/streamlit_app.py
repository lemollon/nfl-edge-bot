...
if prompt:
    ...
    player_news_text = ""
    if include_news and players_list:
        try:
            pitems = fetch_player_news(players_list, team_hint=teams[0] if teams else "", max_items_per_player=3)
...
# =============================================================================
# Latest Headlines (always visible)
# =============================================================================
st.divider()
...
with col_team:
    st.subheader("Team / League")
    try:
        news_items = fetch_news(max_items=10, teams=teams_for_news)
        if not news_items:
...
with col_player:
    ...
    if not players_list:
        ...
        try:
            pitems = fetch_player_news(players_list, team_hint=teams_for_news[0] if teams_for_news else "", max_items_per_player=2)
            if not pitems:
...
st.caption("CSV columns (case-insensitive): Player, Pos, % Rostered")
